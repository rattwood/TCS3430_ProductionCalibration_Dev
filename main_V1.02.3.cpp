...
#define CFG2_REG 0x1F   // CFG2 register (0x9F with CMD_BIT). Bit4 = HGAIN (128x gain enable)
#define CFG3_REG 0x2B   // CFG3 register (0xAB with CMD_BIT). Interrupt read-clear / SAI, etc. (NOT gain)
#define AZ_CONFIG_REG 0x56  // Auto-zero configuration register
#define PERS_REG     0x0C   // Persistence register
#define INTENAB_REG  0x5D   // 0xDD on device (INTENAB: AIEN bit enables ALS interrupt)
...
// NOTE: If your datasheet shows a different bit for "data valid", change this mask.
#define STATUS_AVALID_MASK   0x01  // ALS Data Valid (A VALID)
...
uint8_t INTEG_TIME = 0x09; // ~25 ms (reduced for very bright LED conditions)
...
void setupWiFi() {
  switch (wifiMode) {
    ...
    case 2: { // Dual mode (note the braces)
      Serial.println("[WiFi] Entering Dual Mode");

      WiFi.mode(WIFI_AP_STA);
      delay(100);

      bool apOk = WiFi.softAP("TCS3430-ColorSensor", "");
      ...
      currentIP = WiFi.softAPIP().toString(); // default to AP IP unless STA connects
      ...
      if (wifiSSID.length() > 0) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          // Keep currentIP as STA if you prefer showing station IP:
          // currentIP = WiFi.localIP().toString();
    ...
    default: // Anything else -> AP only
      Serial.println("[WiFi] Invalid wifiMode, defaulting to AP");
      wifiMode = 0;
      startAccessPoint();
      currentIP = WiFi.softAPIP().toString();
      break;
...
{
  // STATUS bits are write-1-to-clear for AINT/ASAT on this part
  writeSingleByte(STATUS_REG, 0x10); // clear AINT
  writeSingleByte(STATUS_REG, 0x80); // clear ASAT
}
...
void setAMUX(bool ir2Mode) {
  ...
  if (ir2Mode) cfg1 |=  (1 << 3);   // AMUX=1 -> IR2 on CH3
  else         cfg1 &= ~(1 << 3);   // AMUX=0 -> X  on CH3
...
static void ensureALSRunning();
static uint16_t readCH3_AdafruitStyleAfterAMUX(bool ir2Mode);

static uint32_t integrationMsFromATIME(uint8_t atime)
{
  // TCS3430: integration ≈ (ATIME + 1) * 2.78ms
  return (uint32_t)((atime + 1) * 2.78f + 0.5f);
}

static void restartALS()
{
  // Keep PON on, toggle AEN to restart integration cycle
  uint8_t en = readSingleByte(ENABLE_REG);
  en |= ENABLE_PON;

  // AEN off
  writeSingleByte(ENABLE_REG, (uint8_t)(en & ~ENABLE_AEN));
  delay(3);
 ...
 static bool waitALSValid(uint32_t timeoutMs)
    {
    (void)timeoutMs;
      // TCS3430 STATUS does not provide a usable "data valid" bit for our purpose.
      // Do not poll STATUS for readiness.
    return true;
    }
...
static void ensureALSRunning() {
  uint8_t en = readSingleByte(ENABLE_REG);
  en |= (ENABLE_PON | ENABLE_AEN);
  writeSingleByte(ENABLE_REG, en);
}
...
static uint16_t readCH3_AdafruitStyleAfterAMUX(bool ir2Mode) {
  ...
  // Wait for a NEW integration result after switching AMUX
  uint8_t atime = readSingleByte(ATIME_REG);
  uint32_t integMs = integrationMsFromATIME(atime);
  delay(integMs + 10);

  // CH3 low register is XDATA_L (CH3DATAL). When AMUX=1 it is IR2.
...
{
  if (DEBUG_IR2_SERIAL) {
    const uint32_t integMs = integrationMsFromATIME(INTEG_TIME);
    Serial.printf("[IR2] ATIME=0x%02X integMs=%lu waitMs=%lu\n",
                  INTEG_TIME, (unsigned long)integMs, (unsigned long)(integMs + 15));
  }

  if (DEBUG_IR2_SERIAL) clearStatusFlags(); // optional, fine to keep for readability of STATUS  

  // Get a truly fresh IR2 sample
  uint16_t ir2 = readIR2RawFresh();

  // Restore AMUX=X and re-sync X pipeline (optional but recommended)
  (void)readXRawFresh();
...
static uint16_t readXRawFresh()
{
  ...
  // Force a new integration with AMUX=X
  restartALS();

  const uint32_t waitMs = integrationMsFromATIME(INTEG_TIME) + 20;
  delay(waitMs);

  // Flush stale sample then read
  (void)read16(XDATA_L);
  return read16(XDATA_L);
}
...
static uint16_t readIR2RawFresh()
{
  ...
  // Force a new integration with AMUX=IR2
  restartALS();
...
void setHighGain(bool enable) {
  uint8_t cfg2 = readSingleByte(CFG2_REG);

  // Preserve other bits, force reserved low nibble to 0x04, set/clear HGAIN (bit4)
  cfg2 = (cfg2 & 0xE0) | 0x04;          // keep bits 7:5, force bits 3:0 = 0100
  if (enable) cfg2 |= 0x10;             // set bit4
  else        cfg2 &= ~0x10;            // clear bit4

  writeSingleByte(CFG2_REG, cfg2);
}
// void setHighGain(bool enable) {
//   Wire.beginTransmission(TCS3430_ADDR);
...
void setup() {
  ...
  if (settingsLoadedOk) {
    ...
    if (saveSettings()) {
      Serial.println("✓ Created default /settings.json in SPIFFS");
    } else {
      Serial.println("⚠ Failed to create default /settings.json");
    }
  }
  ...
  initSensor();
   ...
   // ---- IR2 debug stability: disable ALS interrupts + clear latched flags ----
  writeSingleByte(INTENAB_REG, 0x00);  // disable ALS interrupt so STATUS AINT doesn't latch
  clearStatusFlags();   
  
  uint8_t inten = readSingleByte(INTENAB_REG);
  uint8_t st    = readSingleByte(STATUS_REG);
  Serial.printf("[BOOT] INTENAB=0x%02X  STATUS=0x%02X\n", inten, st);// clear any stuck AINT/ASAT flags
  // -------------------------------------------------------------------------
...
function drawBCRATiles(ctx, width, height) {
  ...
  setInterval(refresh, 10000); // Increased to 10 seconds to allow plenty of time for input entry
    ...
    r->send_P(200, "text/html", measurement_html);
  });
      ...
      <div class="config-panel">
        ...
        <div class="form-group">
          ...
          <select id="integrationSelect" class="form-input">
            <option value="0x09">~28ms (Bright Light)</option>
            <option value="0x12">~53ms (Normal Light)</option>
            <option value="0x23">~100ms (Dim Light)</option>
            <option value="0x40">~180ms (Very Dim)</option>
          </select>
...
<script>
  async function loadDiagnostics() {
    try {
      ...
      const cal = colorData.calibration || {};
      const kx = (typeof cal.k_x === 'number') ? cal.k_x : 0;
      const ky = (typeof cal.k_y === 'number') ? cal.k_y : 0;
      const kz = (typeof cal.k_z === 'number') ? cal.k_z : 0;
      ...
      document.getElementById('calibrationAccuracy').innerHTML = `
        ...
        <div>K-factors: X=${kx.toFixed(4)}, Y=${ky.toFixed(4)}, Z=${kz.toFixed(4)}</div>
      `;
  ...
  function atimeToMs(atime) {
    return Math.round((atime + 1) * 2.78); // typical AMS formula
  }

  function getIntegrationTime(intTime) {
    return atimeToMs(intTime).toString();
  }
  ...
  setInterval(loadDiagnostics, 10000);
    ...
    r->send_P(200, "text/html", diagnostics_html);
  });
...
<script>
  ...
  setInterval(readCurrent, 3000);
    ...
    r->send_P(200, "text/html", calibration_html);
  });
  ...
  server.on("/debug", HTTP_GET, [](AsyncWebServerRequest* r){
    ...
    if (DEBUG_IR2_SERIAL) useIR2 = true;
    ...
    // --- AMUX VERIFY values (so we can print to Serial and also include in HTTP response) ---
uint16_t x_ch3 = 0;
uint16_t ir2_ch3 = 0;
uint16_t diff_ch3 = 0;

  if (DEBUG_IR2_SERIAL) {
  Serial.println("\n--- AMUX VERIFY (Adafruit-style CH3) ---");

  x_ch3   = readCH3_AdafruitStyleAfterAMUX(false);
  ir2_ch3 = readCH3_AdafruitStyleAfterAMUX(true);
 
  // Restore AMUX=X at end
  (void)readCH3_AdafruitStyleAfterAMUX(false);

  diff_ch3 = (ir2_ch3 > x_ch3) ? (ir2_ch3 - x_ch3) : (x_ch3 - ir2_ch3);

  Serial.printf("[AMUX=X]   CH3(X)   = %u\n", x_ch3);
  Serial.printf("[AMUX=IR2] CH3(IR2) = %u\n", ir2_ch3);
  Serial.printf("Diff(|IR2-X|)=%u\n", diff_ch3);
    ...
    bool hgain    = (cfg2 & 0x10) != 0;           // HGAIN is CFG2 bit4
    
    String response = "=== TCS3430 Color Sensor Debug ===\n";

// Include AMUX verify results in the HTTP response when serial debug is enabled
if (DEBUG_IR2_SERIAL) {
  response += "\n--- AMUX VERIFY (Adafruit-style CH3) ---\n";
  response += "CH3(X)   = " + String(x_ch3) + "\n";
  response += "CH3(IR2) = " + String(ir2_ch3) + "\n";
  response += "Diff     = " + String(diff_ch3) + "\n\n";
}
