...
uint16_t readIR2Raw() {
  ...
  const uint32_t waitMs  = integMs + 15;  // guard
  ...
  // Clear any latched flags (optional but helps keep STATUS readable)
  clearStatusFlags();

  // Switch CH3 → IR2
  setAMUX(true);

  // Force a new integration under IR2 mux selection
  restartALS();
  delay(waitMs);
