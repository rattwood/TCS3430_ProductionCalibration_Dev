...
void setAMUX(bool ir2Mode) {
  ...
  // try bit 3 first (current)
  uint8_t bit = 3;
  if (ir2Mode) cfg1 |= (1 << bit);
  else         cfg1 &= ~(1 << bit);
  ...
  if (DEBUG_IR2_SERIAL) {
    Serial.printf("[AMUX] bit=%u set %s | CFG1 before=0x%02X after=0x%02X\n",
                  bit, ir2Mode ? "IR2" : "X", before, after);
  }
}
...
<script>
  ...
  server.on("/debug", HTTP_GET, [](AsyncWebServerRequest* r){
    ...
    if (DEBUG_IR2_SERIAL) {
  Serial.println("\n--- AMUX VERIFY (raw channels) ---");

  // Read channels with AMUX = X (0)
  setAMUX(false);
  restartALS();
  delay(integrationMsFromATIME(INTEG_TIME) + 20);

  uint16_t ch0_x = read16(ZDATA_L);   // CH0
  uint16_t ch1_x = read16(YDATA_L);   // CH1
  uint16_t ch2_x = read16(IR1DATA_L); // CH2
  uint16_t ch3_x = read16(XDATA_L);   // CH3
  uint8_t  cfg1_x = readSingleByte(CFG1_REG);

  Serial.printf("[AMUX=X]  CFG1=0x%02X  CH0(Z)=%u CH1(Y)=%u CH2(IR1)=%u CH3(X)=%u\n",
                cfg1_x, ch0_x, ch1_x, ch2_x, ch3_x);

  // Read channels with AMUX = IR2 (1)
  setAMUX(true);
  restartALS();
  delay(integrationMsFromATIME(INTEG_TIME) + 20);

  uint16_t ch0_i = read16(ZDATA_L);
  uint16_t ch1_i = read16(YDATA_L);
  uint16_t ch2_i = read16(IR1DATA_L);
  uint16_t ch3_i = read16(XDATA_L);
  uint8_t  cfg1_i = readSingleByte(CFG1_REG);

  Serial.printf("[AMUX=IR2] CFG1=0x%02X  CH0(Z)=%u CH1(Y)=%u CH2(IR1)=%u CH3(?)=%u\n",
                cfg1_i, ch0_i, ch1_i, ch2_i, ch3_i);
...
uint16_t ir2_direct = readIR2Raw();
response += "\nIR2 (direct readIR2Raw) = " + String(ir2_direct) + "\n";
